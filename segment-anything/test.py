...
sam = sam_model_registry["vit_b"](checkpoint="sam.pth")
...
def apply_mask(image, mask, alpha_channel=True):#应用并且响应mask alpha_channel=True
    if alpha_channel:
...
def save_masked_image(image, mask, output_dir, filename, crop_mode_):#保存掩盖部分的图像（感兴趣的图像）
    if crop_mode_:
        ...
        # y1_min, y1_max, x1_min, x1_max = y1.min(), y1.max(), x1.min(), x1.max()
        # cropped_image = image[y1_min:y1_max+1, x1_min:x1_max+1]
        # y, x = np.where(mask)
